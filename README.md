...
Learned the List interface and its ordered, duplicate-allowed structure.
Understood the use of ArrayList vs LinkedList.
Practiced reversing lists without built-in methods.
Implemented frequency counting using List + Map.
Solved list rotation and duplicate removal while preserving order.
Found the Nth element from the end in a LinkedList without size calculation.
